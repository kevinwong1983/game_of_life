...
MqttClient::MqttClient(const std::string& client_id,
    ...
    mqtt_client_->set_clean_session(clean_session);
...
void MqttClient::Disconnect(){
    mqtt_client_->async_disconnect([](MQTT_NS::error_code ec){
...
void MqttClient::OnConnected(MQTT_NS::error_code ec) {
    if (!ec.failed()) {
        ...
        for (auto&& item: publications_) {
            mqtt_client_->async_publish(std::get<0>(item), std::get<1>(item), std::get<2>(item).get()); // add retainment, expose qos on interface
        }
        publications_.empty();
    }
}
...
void MqttClient::Subscribe(Topic topic, MessageHandler handler) {
    if (mqtt_client_->connected()){
        DoSubscribe(std::move(topic), std::move(handler));
    }
    else {
...
void MqttClient::Publish(const Topic& topic, const Message& message, MqttPublishOptions options) {
    if (mqtt_client_->connected()){
        mqtt_client_->async_publish(topic, message, std::move(options.get()));
    }
    else {
        publications_.emplace_back(topic, message, std::move(options));
    }
}
